...
* Download version %%ODTK_VERSION%% or later of ODTK for Linux from [AGI Downloads](https://support.agi.com/downloads). Unzip this file and copy the `odtk_binaries_v%%ODTK_VERSION%%.tgz` and `odtk_data_v%%ODTK_VERSION%%.tgz` into the [`distributions`](./distributions) folder at the same level as this file.
...
If you did not build the `custom-environment` image described above, run the following in command line from this directory:
```
docker build -t ansys/odtk/odtk-runtime:%%ODTK_VERSION%%-ubi8 .
```

If you did build the `custom-environment` image described above, run the following in command line from this directory:
```
docker build -t ansys/odtk/odtk-runtime:%%ODTK_VERSION%%-ubi8 --build-arg baseImage=ansys/odtk/custom/redhat/ubi8:latest .
```
...
1. Run the following in command line from this directory:
```
docker run -d -it -p 9393:9393 --env-file ../configuration/licensing.env --name odtk-runtime --rm ansys/odtk/odtk-runtime:%%ODTK_VERSION%%-ubi8
```
> [!NOTE]
...
2. Run the following in command line from this directory:
```
