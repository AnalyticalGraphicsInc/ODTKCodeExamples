...
2. Unzip this file and copy the `odtk_codesamples_v7.9.1.tgz` into the [`distributions`](./distributions) folder at
the same level as this file.
3. Run `docker build -t ansys/odtk/odtk-python:7.9.1-ubi8 .` on the command line in this directory.
...
`docker run -it --env-file ../configuration/licensing.env --name odtk-python --rm ansys/odtk/odtk-python:7.9.1-ubi8`
...
1. Download version 7.4.0 or later of ODTK for Linux from [AGI Downloads](https://support.agi.com/downloads).
2. Unzip this file and copy the `odtk_codesamples_v7.9.1.tgz` into the [`distributions`](./distributions) folder at the same level as this file.
